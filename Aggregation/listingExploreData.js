...
const match_id = async(payloadData,userData) =>{
      try{
            let match = {
                    $match : {
                              _id : { $nin  : [mongoose.Types.ObjectId(userData._id)]},
                              $and: [
...
const list_data = async(payloadData,userData,limits,pageNumber) => {
    try{
        let match = await match_id(payloadData,userData) ;
        ...
        if(aggreated_details.length != 0){
            ...
            for(let data of aggreated_details){
                ...
                if(payloadData.search)
                {
                    ...
                    let questions = payloadData.search.trim() ;
                ...
                if(payloadData.expertIndustry) {
                  let data_for_industry = {
                        expertIndustry : payloadData.expertIndustry,
                        _id  : data._id
                  } ;
                  let fetched_data = await DAO.getData(Models.users,data_for_industry,{ __v : 0 },{ lean : true }) ;

                  if(fetched_data.length != 0) {
                       
                        industryArray.push(data) ;
